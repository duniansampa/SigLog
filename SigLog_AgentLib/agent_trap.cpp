...
#include <siglog/agent/snmp_agent.h>
...
extern const oid       snmptrap_oid[] = { SNMPV2_TRAP_OBJS_PREFIX, 1, 0 };
const oid       snmptrapenterprise_oid[] = { SNMPV2_TRAP_OBJS_PREFIX, 3, 0 };
extern const oid       sysuptime_oid[] = { SNMP_OID_MIB2, 1, 3, 0 };
...
extern const size_t    sysuptime_oid_len = OID_LENGTH(sysuptime_oid);
...
send_trap_to_sess(netsnmp_session * sess, netsnmp_pdu *template_pdu)
{
    ...
    if ( template_pdu->command == SNMP_MSG_INFORM
        ...
        if ((sess->version == SNMP_VERSION_3) &&
            ...
            pdu->securityEngineID = (u_char *) netsnmp_memdup(tmp, len);
...
snmpd_parse_config_trapsess(const char *word, char *cptr)
{
    ...
    if (ss->version == SNMP_VERSION_3 &&
        ...
        ss->securityEngineIDLen == 0) {
            ...
            ss->securityEngineID = (u_char *) netsnmp_memdup(tmp, len);
