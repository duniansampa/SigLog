...
usm_create_user_from_session(netsnmp_session * session)
{
    ...
    if (user == NULL) {
         ...
         */
        user->engineID = (u_char *)netsnmp_memdup(session->securityEngineID,
                                        session->securityEngineIDLen);
    ...
    if (user->authKey == NULL) {
        if (session->securityAuthLocalKey != NULL
            ...
            user->authKey = (u_char *)netsnmp_memdup(session->securityAuthLocalKey,
    ...
    if (user->privKey == NULL) {
        if (session->securityPrivLocalKey != NULL
            ...
            user->privKey = (u_char *)netsnmp_memdup(session->securityPrivLocalKey,
...
usm_parse_create_usmUser(const char *token, char *line)
{
    ...
    if (!cp) {
         ...
         */
        newuser->privKey = (u_char *)netsnmp_memdup(newuser->authKey,
                                          newuser->authKeyLen);
