...
#include <siglog/library/snmp.h>      /* for xdump & {build,parse}_var_op */
#include <siglog/library/snmp_api.h>
#include <siglog/library/snmp_client.h>
#include <siglog/library/parse.h>
...
#include <siglog/library/snmpksm.h>
#endif
...
#include "../SigLog_MibLib/agentx/protocol.h"
...
snmpv3_probe_contextEngineID_rfc5343(void *slp, netsnmp_session *session) {
    ...
    pdu->contextEngineID = (u_char *)netsnmp_memdup(probeEngineID, probeEngineID_len);
    ...
    if (NULL != response->variables &&
        ...
        session->contextEngineID =
            (u_char *)netsnmp_memdup(response->variables->val.string,
                             ...
                             struct timeval *timeout, int *block, int flags)
{
    ...
    for (slp = (sessp ? (struct session_list *)sessp : Sessions); slp; slp = next) {
        next = slp->next;
