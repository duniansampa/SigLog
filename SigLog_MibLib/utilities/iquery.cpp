                              ...
                              void *serverargs, void *clientarg)
{
    ...
    if ( s && s->securityEngineIDLen == 0 ) {
        ...
        s->securityEngineID = (u_char *)netsnmp_memdup(eID, elen);
...
netsnmp_session *netsnmp_iquery_session(char* secName,   int   version,
    ...
    if (ss) {
        ...
        ss->securityEngineID = (u_char *)netsnmp_memdup(engineID, engIDLen);
        ss->securityEngineIDLen = engIDLen;
        if ( version == SNMP_VERSION_3 ) {
            ss->securityNameLen = strlen(secName);
            ss->securityName = (char *)netsnmp_memdup(secName, ss->securityNameLen);
        } else {
            ss->community = (u_char *)netsnmp_memdup(secName, strlen(secName));
